const { readFileSync } = require('fs');
  ...
  function gerarFaturaHTML(fatura, pecas, calc) {
    ...
    for (let apre of fatura.apresentacoes) {
      body += `<li> ${getPeca(apre, pecas).nome}: ${formatarMoeda(calc.calcularTotalApresentacao(apre, pecas))} (${apre.audiencia} assentos) </li>\n`;
    }
    body+=`</ul>\n`
    body += `<p>Valor total: ${formatarMoeda(calc.calcularTotalFatura(fatura.apresentacoes, pecas))}</p>\n`;
    body += `<p>Créditos acumulados: ${calc.calcularTotalCreditos(fatura.apresentacoes, pecas)}</p>\n`;
    body+=`</html> \n`
    return body;
  }
